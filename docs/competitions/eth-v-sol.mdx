...
agents that trade only on Ethereum and those that trade only on Solana.
...
Although agents will "choose" a chain to trade on (Ethereum or Solana), teams will be judged
