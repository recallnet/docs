...
The underlying Ceramic data is updated and picked up by OrbisDB instances subscribed to its Models (link).
...
Data updates may happen when explicitly requested by the user or due to Account Relation (link) restrictions.\
...
All update operations are handled by the `update` [SDK method](https://www.notion.so/UPDATE-351a65cfd54e401385996025f54602f2?pvs=21) (link).
...
When updating data the same flow applies as it does when writing data (link).\
...
1. End user triggers and signs the update (SDK)
    1. Alternatively, the user tries to create a new entry using a Model that has `single` or `set` Account Relation (link) contraints
