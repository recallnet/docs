---
title: Custom tools
description: Create and integrate custom tools with the Recall MCP server
---

While Recall's Agent Toolkit provides a comprehensive set of built-in tools for bucket management,
object storage, and source verification, you may need to extend this functionality with your own
custom tools. This guide shows you how to create and integrate custom MCP tools that work alongside
Recall's built-in capabilities.
...
For Claude Desktop integration, you can modify the configuration to include your custom tool
implementation:
...
<Callout type="info">
  Make sure your custom server script is compiled to JavaScript (if using TypeScript) and properly
...
Your agent should now be able to call your custom `get_weather` tool alongside the built-in Recall
tools.
...
This example shows a custom tool that calls an external API and stores the results in a Recall
bucket:
  ...
  execute: async ({ query, count = 5, bucketName }, context) => {
    try {
      ...
      const content = new File(
        [JSON.stringify({ query, timestamp, articles: response.data.articles })],
        key,
        {
          type: "application/json",
        }
      );
      await client.add(bucket, key, content);
...
```typescript title="test-tools.ts"
import { weatherTool } from "./custom-tools";
  ...
  return {
    ...
    execute: async (params, context) => {
      ...
      try {
        ...
        console.log(
          `[${new Date().toISOString()}] ${tool.name} completed in ${duration}ms`,
          result
        );
        ...
        const duration = Date.now() - startTime;
        console.error(
          `[${new Date().toISOString()}] ${tool.name} failed after ${duration}ms`,
          error
        );
...
export const memoryTool: Tool = {
  ...
  execute: async ({ fact, category }, context) => {
    ...
    try {
      ...
      const content = new File(
        [JSON.stringify({ fact, category, timestamp, created: new Date().toISOString() })],
        key,
        { type: "application/json" }
      );
...
1. Explore the [MCP Protocol Documentation](https://modelcontextprotocol.io/introduction) for deeper
   insights
