...
This page walks through how to spin up a local Hoku network with either Docker (`localnet`) or on
...
If you're using the `hoku` CLI, you must set the `HOKU_NETWORK` environment variable to `devnet` in
order to use it. Note that `fendermint` `ipc-cli` the `NETWORK` flag but with different target
functionality, so be sure the `HOKU_` prefix is attached.
