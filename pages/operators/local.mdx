...
Starting the network handles the build and installation of a few key binaries that help run the
underlying network: `ipc-cli`, `fendermint`, and `cometbft`. The IPC contracts take _a while_ (5-10
minutes) to compile the first time.
...
with the [SDK and CLI](/basin).

You'll want to create an `.env` file that specifies the `NETWORK` as `localnet` (this is defined as
`testnet` by default):
...
For example, you can now run `adm os create` with the [CLI](/basin/cli).
