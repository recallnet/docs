...
Hoku’s core functionality is built on top of the FVM as a series of _data contracts_, which are,
...
"built-in" FVM and "custom" (subnet-specific) data contracts are compiled to CAR files and provided
...
Within Hoku, our default contracts are called **_data contracts_**. These are the core building
blocks of Hoku; each data contract has its own unique address and is associated with a unique owner
address. This helps track which contracts are tied to a given account. At present, Hoku ships with
three core data contracts: the _address manager_, the _object store_, and the _accumulator_. These
contracts are responsible for managing the state of a subnet, creating new object stores, and
accumulators, and storing and retrieving data from the subnet.
...
tree of the subnet. Notably, this means you can create data contracts that _use_ IPLD (e.g., Rust's
...
This flexibility enables the creation of both smart contract and data contracts, the latter of which
...
Hoku comes with a few built-in data contracts that are used to manage the state of the subnet and
implement core data structures.

### Address manager

Clients are able to deploy new object stores on demand using the singleton address manager. The
primary function of the address manager is to create other _data contracts_ and assign them a unique
address. Each data contract is associated with a unique address (which is used to identify the store
...
Object stores are deployed as instances of the object store data contract. This contract is
