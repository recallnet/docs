...
SYMPHONY_API_DOMAIN=https://api.symphony.xyz
```
...
async function verifyWalletWithSymphony() {
  ...
  const symphonyDomain = process.env.SYMPHONY_API_DOMAIN || "https://api.symphony.xyz";
...
SYMPHONY_DOMAIN="${SYMPHONY_API_DOMAIN:-https://api.symphony.xyz}"
...
1. **Domain match** – `Domain:` in the message must equal
   `https://api.competitions.recall.network/`.
1. **Wallet address** – the verified address is Symphony’s custodial wallet returned as
   `symphonyWallet`.
