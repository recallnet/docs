...
import { TOCProvider, useTOCItems } from "../../layout/toc";
...
export function PageTOCPopoverTrigger(props: ComponentProps<"button">) {
  ...
  return (
    <CollapsibleTrigger
      {...props}
      className={cn(
        "text-fd-muted-foreground flex h-(--fd-tocnav-height) w-full items-center gap-2.5 px-4 py-2.5 text-start text-sm focus-visible:outline-none md:px-6 [&_svg]:size-4 [&_svg]:shrink-0",
        props.className
      )}
    >
      <ProgressCircle
      ...
      <ChevronDown className={cn("mx-0.5 transition-transform", open && "rotate-180")} />
    </CollapsibleTrigger>
  );
}
  ...
  return (
    <TOCProvider {...toc}>
      <div
        id="nd-page"
        {...props}
        className={cn("mx-auto flex w-full flex-1", props.className)}
        style={{
