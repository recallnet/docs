...
Recall's testnet chain is currently in beta and **frequently reset every ~2 weeks**. In the near
...
Retrieval augmentation generation (RAG) is a technique that uses a large language model (LLM) to
generate a response to a user query based on a set of retrieved documents. A common pattern is to:

1. Deteremine the set of relevant documents (e.g., some knowledge base), and retrieve them
