...
To create an object store, you'll need to use the `JsonRpcProvider` from the `hoku_provider` crate plus various
`hoku_sdk` types.
...
Lastly, since buckets are a fully mutable store, you can delete an object using the `delete` method. It takes similar
...
You can review the `hoku_sdk::network` module for more information or check out the network configuration docs. Utility
methods exist on the `hoku_sdk::network::Network` struct to fetch network values dynamically, so there's not necessarily
