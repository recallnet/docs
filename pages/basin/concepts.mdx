...
- If you're coming from the EVM world, you'll mostly see two types in Hoku:
  - `t2`/`f2`: Any WASM contract that gets deployed, such as an bucket.
...
enable some other account to draw down from this admin account's credit balance—referred to as the
"receiver" for the approval. You can also further constrain this by defining a "required caller"
that, for example, only lets the receiver write data only for a specific bucket.

Revocations follow the same pattern. You can revoke an approval for a receiver, or a receiver and
caller (e.g., bucket) pair.
...
First, to get HOKU tokens, reach out to the Hoku team.

Since Basin is built on top of Filecoin, you must also have FIL in your account to interact with the
network if you are bridging HOKU tokens into the subnet. Hoku is currently only live on the Filecoin
Calibration network. You can get tFIL via the faucet
