...
keywords: minio, object storage, basin, bucket, buckets, web3, s3
---
...
This will create the bucket called `my-store` under your `basin` alias. But, recall that the bucket
will _actually_ get used under `<0x_address>.<bucket_name>`. The step below shows how to check the
actual bucket name.
