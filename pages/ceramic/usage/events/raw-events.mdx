...
This section is dedicated to the lower-level functionality exposed by the `CeramicClient` class. For most developers looking to get started with Ceramic, it's recommended to reference the [Produce guide](/event-store/usage/produce) which uses models and model isntance documents. Developers who wish to understand and work with lower-level event construction and publishing should reference the current page.

If you've followed our [Getting Started guide](/event-store/usage/installation), you should have a `CeramicClient` connected to a Ceramic node, as well as a `DID` that can sign and publish events. We'll use these to
