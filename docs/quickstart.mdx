...
## Step 4: Run Your MCP Server for Testing
...
<Steps>
  ...
  ### Monitor your agent's storage

  When your agent creates and uses buckets, you'll need to monitor what data is being stored:

  <Callout type="info">
    Every bucket has a unique identifier (e.g., `0xff000000000000000000000000000000000001a0`). Make sure to log or save this ID when your agent creates a bucket.
  </Callout>

  ```typescript
  // Example of logging a bucket ID when creating a bucket
  const result = await toolkit.execute("get_or_create_bucket", {
    alias: "agent-memories"
  });
  console.log("Created bucket with ID:", result.result.bucket);
  ```

  #### Viewing bucket contents

  You have several options to view what data your agent is storing:

  1. **Portal web interface**: Visit `https://portal.recall.network/buckets/{bucketId}`
     - Example: `https://portal.recall.network/buckets/0xFF00000000000000000000000000000000000154`
     - View all objects, their content, and metadata

  2. **Recall CLI**: Use the CLI with your agent's private key:
     ```bash
     recall bucket list  # List all buckets
     recall object list <bucket-id>  # List objects in a specific bucket
     recall object get <bucket-id> <object-key>  # View object content
     ```

  3. **Portal account view**: Log into the portal with the wallet used by your agent to see all activity

  4. **Ask your agent**: Prompt your agent to list its buckets and contents:
     ```
     Please list all bucket IDs you've created and their portal URLs.
     ```

  <Callout type="tip">
    For automated testing, consider adding code to log bucket IDs and portal URLs whenever your agent creates a new bucket.
  </Callout>
  ...
  2. Buy credits with your RECALL tokens (e.g., "Buy 1 RECALL worth of credits")
  3. Create a new bucket
  4. Add data to your bucket
  5. Retrieve data from your bucket
</Steps>
