...
This guide shows you how to integrate the Recall Agent Toolkit with Mastra to build agents with persistent memory and storage capabilities.
...
<Steps>
  ...
  ```bash
  npm install @recallnet/agent-toolkit @mastra/core @mastra/agents
  ```
...
import { RecallAgentToolkit } from "@recallnet/agent-toolkit/mastra";

async function main() {
  // Initialize the Recall toolkit
  ...
  // Get Mastra tools from the toolkit
...
async function main() {
  ...
  const result = await noteWorkflow.execute(
...
class RecallMemory implements Memory {
  ...
  constructor(toolkit: RecallAgentToolkit, bucketAlias: string) {
  ...
  async initialize() {
    ...
    const getBucketTool = this.toolkit.getTools().find(t => t.name === "recall_get_or_create_bucket");
    if (!getBucketTool) {
  ...
  async store(key: string, value: any): Promise<void> {
    ...
    const addObjectTool = this.toolkit.getTools().find(t => t.name === "recall_add_object");
    if (!addObjectTool) {
  ...
  async retrieve(key: string): Promise<any> {
    ...
    const getObjectTool = this.toolkit.getTools().find(t => t.name === "recall_get_object");
    if (!getObjectTool) {
  ...
  async delete(key: string): Promise<void> {
    ...
    const deleteObjectTool = this.toolkit.getTools().find(t => t.name === "recall_delete_object");
    if (!deleteObjectTool) {
  ...
  async clear(): Promise<void> {
    ...
    // List all objects
    const queryObjectsTool = this.toolkit.getTools().find(t => t.name === "recall_query_objects");
    if (!queryObjectsTool) {
...
async function main() {
  ...
  const memory = await new RecallMemory(recallToolkit, "agent-memory").initialize();

  // Create a Mastra agent with Recall tools and memory
  const agent = new Agent({
    name: "MemoryAgent",
    description: "An agent with persistent memory using Recall",
    tools: recallToolkit.getTools(),
    model: "gpt-4-turbo",
    memory,
  });
...
async function createMultiAgentSystem() {
  ...
  const getBucketTool = toolkit.getTools().find(t => t.name === "recall_get_or_create_bucket");
