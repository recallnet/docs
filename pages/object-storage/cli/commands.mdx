...
The Hoku CLI is a command-line interface for interacting with the Hoku network. It allows you to
manage your accounts, interact with smart contracts, and manage your data that gets stored on the
network.

## Global options

All the global flags can also be passed as all-caps (prefixed with `HOKU_`), snake case environment
variables that are set, or create and source them in a `env` file. For example, `--network` =>
`HOKU_NETWORK`, `--private-key` => `HOKU_PRIVATE_KEY`, etc.

-n, --network <NETWORK> Network presets for subnet and RPC URLs [env: HOKU_NETWORK=] [default:
ignition]
[possible values: mainnet, testnet, localnet, devnet, ignition] -s, --subnet <SUBNET> The ID of the
target subnet [env: HOKU_SUBNET=] --rpc-url <RPC_URL> Node CometBFT RPC URL [env: HOKU_RPC_URL=] -v,
--verbosity... Logging verbosity (repeat for more verbose logging) [env: HOKU_LOG_VERBOSITY=] -q, --quiet
Silence logging [env: HOKU_LOG_QUIET=] -h, --help Print help (see more with '--help')

| Flag              | Description                                                                                                        | Env. variable      |
| ----------------- | ------------------------------------------------------------------------------------------------------------------ | ------------------ |
| `-n`, `--network` | Network presets for subnet and RPC: `mainnet`, `ignition`, `testnet`, `localnet`,or `devnet` (default: `ignition`) | HOKU_NETWORK       |
| `-s`, `--subnet`  | The ID of the target subnet.                                                                                       | HOKU_SUBNET        |
| `--rpc-url`       | Node CometBFT RPC URL.                                                                                             | HOKU_RPC_URL       |
| `-v, --verbosity` | Logging verbosity (none: error; `-v`: warn; `-vv`: info; `-vvv`: debug; `-vvvv`: trace).                           | HOKU_LOG_VERBOSITY |
| `-q, --quiet`     | Silence logging (default: `false`).                                                                                | HOKU_LOG_QUIET     |
| `-h, --help`      | Print help.                                                                                                        | -                  |
...
- `HOKU_PRIVATE_KEY`: A wallet private key (ECDSA, secp256k1) for signing transactions.
- `HOKU_NETWORK`: Specify the chain location with RPC presets and settings that map to either
  `mainnet` (not available), `ignition`, `testnet`, `localnet`, or `devnet`.

As such, the examples below omit these flags and presume that you've already set them.

## Account management

Interaction with the Hoku network requires an account (ECDSA, secp256k1). As with any blockchain
system, an account can be created at will, transfer funds, and send transactions. Recall that Hoku
...
build on Hoku, you start by moving funds from the parent L1 (rootnet) to the Hoku subnet. This goes
through a smart contract (i.e., the gateway contract) call that locks the funds in a smart contract
...
to the provided private key. Note that the global `--network` flag (defaults to `ignition`) will set
...
parent. Note that the global `--network` flag (defaults to `ignition`) will set all of the
...
that the global `--network` flag (defaults to `ignition`) will set all of the `--evm-...` presets
for you, but you _can_ override them with your own values, if needed. This transaction happens fully
...
- Buy credits with 0.1 native coins (displayed with $10^(17)$ units).
...
```
hoku credit approve [OPTIONS] --receiver <RECEIVER>
```

| Flag                    | Required? | Description                                                                                                                         |
| ----------------------- | --------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| `-p, --private-key`     | Yes       | Wallet private key (ECDSA, secp256k1) for signing transactions.                                                                     |
| `--receiver <RECEIVER>` | Yes       | The receiver account address.                                                                                                       |
| `--caller <CALLER>`     | No        | Restrict the approval to a caller address, e.g., a bucket. The receiver will only be able to use the approval via a caller contract |
...
```
> hoku credit approve --receiver 0x15d34aaf54267db7d7c367839aaf71a00a2c6a65
...
--caller t2l6pceh34a2v55df2ne5ourud5sputio6m57u2hi
```

- Approve a receiver to use credits for a specific bucket and both a TTL and a credit limit.
...
```
hoku credit revoke [OPTIONS] --receiver <RECEIVER>
```
...
- If a required caller was initially set, it must be provided to revoke the approval.
...
Contracts are the core building blocks of the Hoku network. The `machine` command allows you to
retrieve data contract information relative to a specific address. This helps track which bucket or
accumulator contracts are tied to your account, which are later used in the `bucket` and
`accumulator` subcommands described below.
...
- `hoku bucket list --private-key <PRIVATE_KEY>`: Query with a private key (or read from your `.env`
  file).
...
```
> hoku bucket list --height 789471
[
  {
    "address": "t2l6pceh34a2v55df2ne5ourud5sputio6m57u2hi",
    "kind": "Bucket",
...
And then push it to the bucket:
