...
import { SidebarLinkItem, type SidebarOptions, getSidebarTabsFromOptions } from "./docs/shared";
import { BaseLinkItem, type LinkItemType } from "./links";
import { LayoutBody, LayoutTabs, Navbar, NavbarSidebarTrigger } from "./notebook-client";
      ...
      <NavProvider transparentMode={transparentMode}>
        ...
        >
          <Sidebar
            {...sidebar}
            className={cn(
              navMode === "top" ? "border-e-0 md:bg-transparent" : "md:[--fd-nav-height:0px]",
              sidebar.className
            )}
          >
            <HideIfEmpty>
            ...
            <HideIfEmpty>
              <SidebarFooter className="flex flex-row items-center justify-end">
                <div className="flex flex-1 items-center empty:hidden lg:hidden">
                  {links
                    .filter((item) => item.type === "icon")
                    .map((item, i) => (
                      <BaseLinkItem
...
function DocsNavbar({
  ...
  return (
    <Navbar mode={navMode}>
      <div className={cn("flex flex-1 border-b px-4", navMode === "auto" && "md:px-6")}>
        ...
        <div className="flex flex-1 flex-row items-center justify-end">
          ...
          <NavbarSidebarTrigger className="-me-1.5 p-2 md:hidden" />
          {links
            .filter((item) => item.type === "icon")
            .map((item, i) => (
              <BaseLinkItem
      ...
      {tabs.length > 0 && (
        <LayoutTabs className="h-10 border-b px-6 max-lg:hidden" options={tabs} />
      )}
    </Navbar>
  );
}
