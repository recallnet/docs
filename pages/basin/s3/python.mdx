...
keywords: python, object storage, basin, bucket, buckets, web3, s3
---
...
Assuming you already have a Python environment, install boto3:

<Tabs items={["pip", "uv"]}>
  <Tabs.Tab>pip install boto3</Tabs.Tab>
  <Tabs.Tab>uv pip install boto3</Tabs.Tab>
</Tabs>
...
def connect_to_basin(host: str, access_key: str, secret_key: str) -> S3Client:
    try:
        ...
        print(f"Failed to connect to Basin: {str(e)}")
        raise
```
...
public key. This is important because the adapter has a slight workaround to work with Basin. Any
...
if basin is None:
    raise RuntimeError("Failed to connect to Basin")

# Create a bucket
bucket = create_bucket(basin, "my-bucket", owner_address)
if bucket is None:
...
write_object(basin, bucket, key, b"hello")
print(f"Uploaded file to Basin at key: {key}")

print("Waiting for transaction to settle...")
sleep(3)

# List objects
objects = list_objects(basin, bucket, f"{key_prefix}/")
if not objects:
...
object_data = get_object(basin, bucket, object_key)
if object_data:
