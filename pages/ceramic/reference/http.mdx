...
on the type of event. Visit the [Raw Events guide](/event-store/usage/events/raw-events) for more.
...
You can register an interest by sending a POST request to `interests`. The body of the request contains the fields sep and sepValue.

```jsx
{
  "sep": "model",
  "sepValue": "example-model-id"
}
```

### Read Events Related to an Interest

You can also read events related to an interest by sending a GET request using the `sep` and `sepValue` as part of the URL path, with optional parameters like `controller`, `streamId`, `offset`, and limit.

```jsx
curl -X GET $CERAMIC_API/experimental/events/model/example-model-id?controller=example-did
```
