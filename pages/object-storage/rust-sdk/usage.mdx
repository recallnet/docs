...
To create an bucket, you'll need to use the `JsonRpcProvider` from the `hoku_provider` crate plus
various `hoku_sdk` types.
...
- Although this defined desired `WriteAccess` controls, the actual access is controlled by the
  credit approval logic. Use `WriteAccess::OnlyOwner` for now, but this may change in the future.
- A `HashMap` of metadata to assign to the bucket.
- Lastly, optional gas parameters (these can be imported from `hoku_provider::message::GasParams`).
...
async fn main() {
    ...
    let tx = bucket
        .add(&provider, &mut signer, key, file.file_path(), options)
        .await?;
}
```

For reference, a transaction includes the status, hash, block height, gas used, and the object's
information and metadata.

```json
{
  "hash": "1CAF965B9C77D85F8A87FE8097E0912441E74D52ABD2BCC03DC39D47F23E207A",
  "height": "70312",
  "gas_used": 249175079,
  "status": "committed",
  "object": {
    "hash": "lioj4xmf3geyffydp2hcj5u3wdtajkcmshb3h32hqsrxjajjadmq",
    "recovery_hash": "tczmmjk737gya3unto6qrfuomnykwyynan7cx3i355r2ivt7bbpq",
    "size": 1024,
    "expiry": 73912,
    "metadata": {
...
```sh copy
curl https://evm-ignition-0.hoku.sh \
-X POST \
