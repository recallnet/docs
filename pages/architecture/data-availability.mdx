...
Each new data file/blob that is added to a Hoku bucket machine via a detached payload transaction
...
of its object metadata. Given that time in Hoku is measured in block time, this retention policy is
specified as a TTL in blocks (e.g., TTL = 5). For a file/blob added at block 10, with a TTL of (at
least) 5 blocks, we then assign it to _retention group_ 15. When entangling data, validators must
always entangle incoming data (and associated chunks) with pseudorandom chunks from the correct
