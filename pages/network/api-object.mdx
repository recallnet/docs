---
title: Object API
description: Endpoints for interacting with the Hoku network's object API.
---

import { Callout } from "nextra/components";

The object API exposes a set of endpoints for interacting with the Hoku network's blob storage.
Buckets are an abstraction around blobs the help you manage these blobs. Under the hood, Hoku nodes
use [Iroh](https://www.iroh.computer/) and expose the HTTP API to allow anyone to upload signed data
to the Hoku network.

### Base URL

The object API endpoints for each of the Hoku chain environments are listed below:

- `testnet`: `https://object-api-ignition-0.hoku.sh`
...
When you upload data to the Hoku network, you will need to know the information above to properly
...
export BUCKET_ID=$(echo $BUCKET | sed 's/0xff00*//g' | xargs -I {} printf "%d" "0x{}")
```

Then, we'll download the object. The API currently expects the FVM-style bucket ID address, which is
a `f0` or `t0` prefixed before the bucket ID's decimal value.

```bash
curl -X GET http://localhost:8001/v1/objects/t0$BUCKET_ID/$OBJECT_KEY > downloaded.txt
```
...
directly and, instead, use the Hoku CLI, SDK, or other clients that help you manage the process.

The upload endpoint is designed to accept multipart form data with the following fields:

- `chain_id`: The chain ID for Hoku.
...
The `metadata_hash` is a blake3 hash used by Hoku for data availability and recovery purposes.

<Callout type="info">

We'll put out more detailed examples for how to do this in the future. For now, it's best to rely on
the Hoku clients that handle the upload process.
