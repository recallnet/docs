...
Create `plugins/plugin-recall-trade.ts`:

```ts filename="plugins/plugin-recall-trade.ts" showLineNumbers copy
import type {
  Plugin,
  Action,
  ActionResult,
...
import tradePlugin from "./plugins/plugin-recall-trade.ts";
