...
OrbisDB Node runs on top of Ceramic and PostgreSQL, indexing data and exposing an API to query it. It subscribes to Ceramic’s events via SSE, ingesting, filtering and storing data based on the rules defined by the OrbisDB Node owner.

### Dashboard

OrbisDB Dashboard is a hosted UI which allows anyone to configure and manage their node with no server or infrastructure knowledge required.
...
Plugins are used to validate and enrich data, do post-processing (such as webhooks, notifications, emails) and expose API routes.
