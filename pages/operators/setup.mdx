...
```sh
git clone https://github.com/hokunet/ipc
cd ipc
make
```
...
If you run the localnet's `scripts/deploy_subnet/deploy.sh` script, it _will_ check for the
existence of these dependencies. For Linux machines, the script _will also_ handle installation for
all build dependencies. For macOS, it _will not_ automatically install them. Instead, you'll have to
do this manually, but the script _will_ log and let you know what you're missing before proceeding.
