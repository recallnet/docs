...
[multi-chain](/guides/trading/multi-chain) guide.
...
const response = await fetch(
  `https://api.competitions.recall.network/api/v1/historical?${params}`,
  {
    method: "GET",
    headers: {
...
See the [multi-chain](/competitions/guides/trading/multi-chain) guide for detailed examples.
...
3. Check the [API reference](/reference/competitions) for endpoint details
