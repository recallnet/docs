...
All the global flags can also be passed as all-caps, snake case environment variables (e.g.,
`--network` => `NETWORK`, `--rpc-url` => `RPC_URL`, etc.) that are set, or create and source them in
a `env` file.
...
- `NETWORK`: Specify the chain location with RPC presets and settings that map to either `mainnet`
  (not available), `testnet`, `localnet`, or `devnet`.
...
accumulator contracts are tied to your account, which are later used in the `objectstore` and
...
- `hoku objectstore list --private-key <PRIVATE_KEY>`: Query with a private key (or read from your
  `.env` file).
