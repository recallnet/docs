...
The Hoku system facilitates new data-driven applications by connecting various participants in the
...
- **Infrastructure providers**: Decentralized node operators that operate the network and ensure
  data availability.
- **Data consumers**: Users who interact with applications built on Hoku, consuming or generating
  data.
...
The Hoku datachain provides novel data availability mechanisms for large volumes of heterogenous
...
Hoku also includes a novel _credits_ system to support usage-based pricing and predictable costs.
...
At its core, Hoku exposes _functions_, which are fundamental building blocks for task execution on
...
etc.) contracts that interact with verifiable offchain data stored in the Hoku datachain. There are
...
You can create new types of contracts that suit your data needs. Buckets and accumulators power the
existing offchain data services below, which is why they come as part of the core Hoku system.
...
interface for facilitating its usage. The first two data services on Hoku include:

- **Ceramic**: Offchain event-log service that relies on the onchain _accumulator_ contract for
  anchoring _events_.
- **Basin**: Provides an S3-compatible object storage service by utilizing the onchain buckets
  contract.
...
There are a few limitations to be aware of when building on Hoku. Keep in mind that services like
