...
Querying the object store at this point will show resolved status. The whole process is
asynchronous, and the object is not considered resolved until the next block. See figure for a
high-level overview of the block production process in Hoku based on the CometBFT ABCI++ framework.
